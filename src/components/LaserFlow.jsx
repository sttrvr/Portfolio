    ...
    useEffect(() => {
        ...
        const adjustDprIfNeeded = now => {
            ...
            if (avgFps < lowerThresh) {
                next = clampVal(currentDprRef.current * 0.85, dprFloor, base);
            } else if (avgFps > upperThresh && currentDprRef.current < base) {
                next = clampVal(currentDprRef.current * 1.1, dprFloor, base);
            }
        ...
        return () => {
            ...
            material.dispose();
            renderer.dispose();
            if (mount && canvas && mount.contains(canvas)) mount.removeChild(canvas);
        };
